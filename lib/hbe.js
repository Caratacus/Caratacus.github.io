...
const knownPrefix = "<hbe-prefix></hbe-prefix>";
  ...
  async function decrypt(decryptKey, iv, hmacKey) {
    ...
    const result = await cryptoObj.subtle.decrypt({
      ...
      const hbePass = document.getElementById('hbePass');
      if (hbePass) {
        hbePass.value = '';
        const oldPlaceholder = hbePass.placeholder;
        hbePass.placeholder = wrongPassMessage;
        hbePass.classList.add('hbe-input-error');
        setTimeout(() => {
          hbePass.classList.remove('hbe-input-error');
          hbePass.placeholder = oldPlaceholder || '';
        }, 3000);
      } else {
